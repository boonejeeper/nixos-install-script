...
# flake.nix — generated by nixos-install-from-cachyos.sh  (Wi-Fi variant)
{
  ...
  let system = "x86_64-linux"; in
  {
    ...
    # Wi-Fi kexec installer — built once, used during installation, ephemeral
    packages.\${system}.kexec-wifi =
      (nixpkgs.lib.nixosSystem {
        inherit system;
        modules = [ ./kexec-wifi-image.nix ];
...
# ── kexec-wifi-image.nix (Wi-Fi only) ──────────────────────────────────────────
if [[ "$USE_WIFI" == "true" ]]; then
    HIDDEN_EXTRA=""
    [[ "$WIFI_HIDDEN" == "true" ]] && HIDDEN_EXTRA=$'\nHidden=true'
    cat > kexec-wifi-image.nix <<NIXEOF
# kexec-wifi-image.nix — generated by nixos-install-from-cachyos.sh
# Ephemeral installer: boots into RAM, connects via iwd, installs NixOS, reboots.
# !! Contains Wi-Fi password in plaintext — do NOT commit to a public repository !!
{ modulesPath, lib, pkgs, ... }:
{
  ...
  # Pre-baked iwd profile — filename is SSID with spaces replaced by underscores
  environment.etc."iwd/${WIFI_PROFILE_NAME}.psk".text = ''
    [Security]
    Passphrase=${WIFI_PASSWORD}${HIDDEN_EXTRA}
  '';
  ...
  # Required for kexec boot script
  environment.systemPackages = [ pkgs.kexec-tools ];
}
NIXEOF
    ok "Generated kexec-wifi-image.nix"
fi
...
# ═══════════════════════════════════════════════════════════════════════════════
# 5 — Build Wi-Fi kexec image (if needed)
# ═══════════════════════════════════════════════════════════════════════════════
KEXEC_FLAG=""
if [[ "$USE_WIFI" == "true" ]]; then
    blank; hr; info "5/8  Wi-Fi kexec image"; hr
    if [[ "$DRY_RUN" == "true" ]]; then
        info "DRY RUN — skipping kexec image build."
        info "When running live, this builds: nix build ${WORKDIR}#kexec-wifi"
        KEXEC_FLAG="--kexec ${WORKDIR}/kexec-wifi-installer.tar.gz"
    elif [[ "$VM_TEST" == "true" ]]; then
        info "VM TEST — skipping kexec image build."
        info "(VM test uses nixos-anywhere's built-in QEMU; no custom kexec needed)"
    else
        info "Building Wi-Fi kexec image (downloads ~400 MB from cache.nixos.org)…"
        nix build .#kexec-wifi
        mkdir -p kexec-build/kexec
        cp -L result/bzImage kexec-build/kexec/
        cp -L result/initrd.gz kexec-build/kexec/
        cp -L result/kexec-boot kexec-build/kexec/run
    ...
    blank; hr; info "5/8  Wi-Fi kexec image — skipped (Ethernet)"; hr
fi
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    [[ -n "$KEXEC_FLAG" ]] && NIXOS_ANYWHERE_CMD+=("--kexec" "${WORKDIR}/kexec-wifi-installer.tar.gz")
